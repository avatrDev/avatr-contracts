...
contract Vesting is Context, ReentrancyGuard {
    ...
    function updateAllocationPlus(uint256 nftId, uint256 _lockReleasable) internal {
        allocations[nftId].released += _lockReleasable;
    }


    
     function release(uint256 nftId) external nonReentrant {
        address beneficiary = nftContract.ownerOf(nftId);  // Get the owner of the NFT
        ...
        if (_vestReleasable > 0) {            
            updateAllocationPlus(nftId,_vestReleasable);
    ...
    {
        amount = allocations[nftId].amount;
        lockAmount = (amount * lockBps) / 10000;
        vestAmount = (amount * vestBps) / 10000;

        released = allocations[nftId].released;
